document.addEventListener("DOMContentLoaded", () => {
    ...
    tabBtns.forEach((btn) => {
        btn.addEventListener("click", () => {
            ...
            tabContents.forEach((content) => {
                if (content.dataset.tabGrp === tabGrp) content.classList.remove("active");
    ...
    function processRawInput() {
        ...
        inputText.value = sortCheckbox.checked
            ? sortVars(rawInputReadText.value) + "\n" + sortVars(rawInputText.value)
            : str;
    }
    ...
    processBtn.addEventListener("click", () => {
        processRawInput();
        const variables = getVars(inputText.value, parseInt(startAddress.value) || 0);

        const readVariables = getVars(rawInputReadText.value, parseInt(startAddress.value) || 0);
        // Çıktıları güncelle
        ...
        outputDefinitions2.value = variables
            .map(
                (v) =>
                    `public IVariable ${v.name} { get; set; } = ${v.getVariableCreationString2(
                        read(v),
                    )};`,
            )
            .join("\n");
